...
vec4 get_light_contribution_helper(vec4 diffuse_color, vec3 specular_color, vec3 normal,
...
vec4 get_directional_light_contribution(vec4 diffuse_color, vec3 specular_color, vec3 normal) {
  ...
  return get_light_contribution_helper(diffuse_color, specular_color, normal, incoming_irradiance,
      reversed_direction);
}
...
void main() {
  ...
  if (directional_light.enabled) {
    total_color += get_directional_light_contribution(diffuse_color, specular_color, normal);
  }
  if (point_lights[0].enabled) {
    total_color += get_point_light_contribution(0, diffuse_color, specular_color, normal);
  }
  if (point_lights[1].enabled) {
    total_color += get_point_light_contribution(1, diffuse_color, specular_color, normal);
  }
