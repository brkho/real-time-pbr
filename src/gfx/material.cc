...
void gfx::Material::UseMaterial(GLuint program) {
  ...
  BindMap(program, albedo_info, "albedo", 2);
  BindMap(program, metallic_info, "metallic", 3);
  BindMap(program, roughness_info, "roughness", 4);
  BindMap(program, normal_info, "normal", 5);
  BindMap(program, ao_info, "ao", 6);
}
